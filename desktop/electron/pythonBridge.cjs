...
class PythonBridge {
  ...
  resolvePython() {
    if (process.env.ANALIZ_PYTHON) return process.env.ANALIZ_PYTHON;
    if (process.platform === 'win32') {
      const packaged = path.join(process.resourcesPath || '', 'backend', 'AnalizIstoriiBackend.exe');
      if (fs.existsSync(packaged)) return packaged;
    } else {
      const packaged = path.join(process.resourcesPath || '', 'backend', 'AnalizIstoriiBackend');
      if (fs.existsSync(packaged)) return packaged;
    }
  ...
  projectRoot() {
    ...
    if (process.resourcesPath && !process.env.ANALIZ_FORCE_REPO_ROOT) {
  ...
  async _start() {
    ...
    const root = this.projectRoot();
    const isPackagedExe =
      python.endsWith('AnalizIstoriiBackend.exe') || python.endsWith('AnalizIstoriiBackend');
   ...
   */
  async rpc(method, params = {}, timeoutMs = 180000) {
